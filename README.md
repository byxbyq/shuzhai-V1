...
- **许可证**：`LICENSE` — **GNU AGPL-3.0**（Affero GPL v3）。使用、复制、修改、分发须遵循 AGPL-3.0 全部条款；修改版本必须以 AGPL-3.0 重新授权并公开源码，以网络服务形式对外提供时须向用户提供对应完整源码（AGPL 网络条款）。商业闭源授权另行协商，详见 LICENSE 正文。
