...
# ═══════════════════════════════════════════
# 通用 AI 验证端点 + Prompt 构建器
# ═══════════════════════════════════════════

router = APIRouter()

@router.post("/{check_type}")
def validate_item(check_type: str, data: ValidateRequest):
    ...
    builder = prompts.get(check_type)
    if not builder:
        return err(ErrorCode.INTERNAL_ERROR, f"unknown check type: {check_type}")
    ctx = data.context or {}
    if memory:
        ctx["memory"] = memory
    ...
    try:
        ...
        parsed = _parse_issue_text(result)
        return {
            "ok": True,
            "result": result,
